...
class Klf:  
    def init():#Initialize precipitation data
        ...
        #Klf.q_ref = np.array([1E-6,1E-9,100.,6.726449064255413e-07,-5.,1E-5,1E-8,1E-3])
    ...
    def est_h0(ccfs1,ccf_r):
        ...
        h0 /= 9.
        return(h0)
    def eKlf(ccfs_all,ccf_r,delta,ts,te,β,h0,Q0,Pt=np.array([[1E-5,0.],[0.,1E-8]]),at0=np.array([1.,0.])): #β is the precipitation effect
        mask0 = [False]*1024
...
"""
   def cal_gwl2(at, Pt,thresh,mask_param): #Moveing averageo of gwl
        with open("./gwl_ebino_seikei.dat","r") as f:
            lines = f.readlines()
        gwl = np.array([float(line.split()[4]) for line in lines])
        ...
        b = signal.hanning(7)
        b = b/b.sum()
        gwl = signal.convolve(gwl, b)[:len(gwl)]
        ...
        ccf_gwl = u0[mask_param[0:number]].dot(u1[mask_param[0:number]])
        γ = ccf_gwl/u1[mask_param[0:number]].dot(u1[mask_param[0:number]])
        res = -2*ccf_gwl*γ + γ*γ*u1[mask_param[0:number]].dot(u1[mask_param[0:number]]) + u0[mask_param[0:number]].dot(u0[mask_param[0:number]])
        β = (gwl[num:number+num]).copy()
        β = β-β.mean()
        number = min(len(ave),len(β))
        ave = ave[0:number]
